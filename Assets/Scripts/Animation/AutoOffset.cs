...
public class AutoOffset : MonoBehaviour
{
    ...
    [Header("Click on this tickbox to center this object's origin")]
    public bool centerOrigin; // click this tickbox to center this object's origin.

    [Header("Click on the tick box to scale object onto Target X3D file")]
    public bool scale;

    [Header("Click on the tick box to snap object onto Target X3D file")]
    ...
    void GetParsedTransforms(){
        if (Geometry.GetComponentInChildren<MeshRenderer>()){
                meshRenderer = Geometry.transform.Find("swPRT").GetComponentInChildren<MeshRenderer>();
                //Debug.Log("has MeshRenderer");
                }
    ...
    {
        // This generates a preview for the render bounds of an object. Useful for debugging auto offset scripts.
        if (this.GetComponent<MeshRenderer>()){

            Vector3 center = this.GetComponent<MeshRenderer>().bounds.center;
            Vector3 radius = this.GetComponent<MeshRenderer>().bounds.extents;
        ...
        if (this.GetComponent<SkinnedMeshRenderer>()){
            Vector3 center = this.GetComponentInChildren<SkinnedMeshRenderer>().bounds.center;
            Vector3 radius = this.GetComponentInChildren<SkinnedMeshRenderer>().bounds.extents;
    ...
    /// <summary>
     /// DO NOT USE: CURRENTLY BROKEN. Method for optimizing complex models with many parts. The idea is to create larger, unified models instead of having many smaller ones. 
     /// </summary>
     void CombineMesh() {
 ...
 //                Object.Destroy(bipRoot);
    ...
    ///<summary>
    /// This script moves the origin of the object so that the transform handle is in the center of the renderbounds.
    ///</summary>
    ///<param name="joint"> Reference to joint on which animation is applied, is usually displaced </param>
    public void CenterOrigin(Transform joint){

        Transform offset = joint.transform;

        this.transform.localPosition = -offset.localPosition;
    ...
    public void Scale()
    {
        if (Geometry)
        {
            if (Geometry.GetComponent<SkinnedMeshRenderer>())
            {
                ...
                if (ScaleTarget){
                    GameObject partGeometry = ScaleTarget; //TODO: automate this
                ...
                float scaleDifference =  partScaleX/scaleToSizeX;
    ...
    private void Snap(){
        ...
        Vector3 targetlocation = ScaleTarget.GetComponent<Renderer>().bounds.center;

        transform.position += targetlocation - (Geometry.GetComponent<SkinnedMeshRenderer>().bounds.center );
    }
    ...
    void Update()
    {
        ...
        if (centerOrigin){
            if(transform.parent.name != transform.name + "_handle"){
                GameObject handle = new GameObject(this.name + "_handle");
                handle.transform.SetPositionAndRotation(this.transform.position, this.transform.rotation);
                handle.transform.SetParent(this.transform.parent);
                transform.SetParent(handle.transform);
            }
            CenterOrigin( transform.GetChild(0)); // FIRST CHILD MUST BE THE BONE TRANSFORM
            centerOrigin = false;
        }
        if (scale){
            Scale();
